...
import app.ibiocd.appointment.home.view.components.CardLabAppointment
import app.ibiocd.appointment.home.view.components.LoadingCardLabAppointment
...
@Composable
fun ListLaboratory(appointments: List<Appointment>,
                   files: List<Files>,
        ...
        .background(Color.White))
    {
        ...
        items(count = itemCount) { index ->
            ...
            if (itemCount!=0){
                ...
                val file= files.firstOrNull { it._id == appointments[auxIndex].files && it.laboratory != "" }


                if (file != null && appointmentstudio.files==file._id) {
                    if (medical!=null){
                        val medicalLab= medical.firstOrNull { it._id == appointments[auxIndex].medical  }

                        if (medicalLab != null) {
                            CardLabAppointment(appointmentstudio,file,medicalLab.name_last, onOpenAppointment = {
                    ...
                    if (patient!=null){
                        ...
                        if (patientstudio != null) {
                            CardLabAppointment(appointmentstudio,file,patientstudio.name_last, onOpenAppointment = {
                                val intent = Intent(context,
                                    AppointmentActivity::class.java)
                                intent.putExtra("medicals","$Medical")
                                intent.putExtra("id", ID)
